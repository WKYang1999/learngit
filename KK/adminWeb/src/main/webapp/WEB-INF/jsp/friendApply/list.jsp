                ...
                <div class="col-xs-12">
                    <div class="box box-success">
                        ...
                        <div class="box-body">
                            ...
                            <div id="toolbar">
                                <button type="button" data-ope="delete" class="btn bg-maroon btn-flat">不同意</button>
                                <button type="button" data-ope="update" class="btn bg-olive btn-flat">同意</button>
                            </div>
                            <table id="friendApply_table"></table>
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    $(function(){
        var $friendApplyTable;
        var userId = document.getElementById("userId").value;
        var queryURL="${pageContext.request.contextPath}/friendApplys/"+userId+"/xx";
        $friendApplyTable = $('#friendApply_table').bootstrapTable({
            url: queryURL,
            method: 'GET',
        ...
        $("#searchForm").on("submit",function(){
            $friendApplyTable.bootstrapTable('refresh');
            return false;
        });
        ...
        $("#toolbar").on("click","button",function(){
            ...
            if(ope=="delete"){
                //获取选中的
                var idArray = $friendApplyTable.bootstrapTable('getAllSelections');
                if(idArray.length==0){
                    alert("至少选择一个");
                    return;
                }
                ...
                $.ajax({
                    type:"post",
                    url:"${pageContext.request.contextPath}/friendApplys/"+idArray.join(),
                    data:"_method=delete",
                ...
                $.ajax({
                    type:"post",
                    url:"${pageContext.request.contextPath}/friendApplys/delete/"+idArray.join(),
                    data:"_method=delete",
                    success:function(respData){
                        if(respData.code==200){
                            alert(respData.msg);
