                ...
                <div class="col-xs-12">
                    <div class="box box-success">
                        ...
                        <div class="box-body">
                                    ...
                                    <form id="searchForm" action="" onsubmit="return false;" class="form-inline">
                                        <div class="form-group">
                                            <select class="form-control" name="name">
                                                ...
                                                <option value="reportVideoName">名称</option>
                                            </select>
                                        </div>
                                        <div class="form-group">
                            ...
                            <div id="toolbar">
                                <button type="button" data-ope="delete" class="btn bg-olive btn-flat">举报无效</button>
                                <button type="button" data-ope="update" class="btn bg-blue btn-flat">下架受理</button>
                            </div>
                            <table id="reportVideo_table"></table>
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    $(function(){
        var $reportVideoTable;
        var queryURL="${pageContext.request.contextPath}/reportVideos/xx";
        $reportVideoTable = $('#reportVideo_table').bootstrapTable({
            url: queryURL,
            method: 'GET',
            ...
            columns: [
                {
                    ...
                    formatter : function(value,row,index){
                        if(row.videoId != 0){
                            return "<a href='${pageContext.request.contextPath}/videos/"+row.videoId+"/sp' target=\"_blank\">"+value+"</a>";
                        }else{
                            return value;
                        }
                    }
        ...
        $("#searchForm").on("submit",function(){
            $reportVideoTable.bootstrapTable('refresh');
            return false;
        });
        ...
        $("#toolbar").on("click","button",function(){
            ...
            if(ope=="delete"){
                //获取选中的
                var idArray = $reportVideoTable.bootstrapTable('getAllSelections');
                if(idArray.length==0){
                    alert("至少选择一个");
                    return;
                }
                ...
                $.ajax({
                    type:"post",
                    url:"${pageContext.request.contextPath}/reportVideos/"+idArray.join(),
                    data:"_method=delete",
                ...
                $.ajax({
                    type:"post",
                    url:"${pageContext.request.contextPath}/reportVideos/delete/"+idArray.join(),
                    data:"_method=delete",
                    success:function(respData){
                        if(respData.code==200){
                            $reportVideoTable.bootstrapTable('refresh');
