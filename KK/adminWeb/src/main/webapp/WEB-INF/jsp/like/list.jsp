...
<script type="text/javascript">
    $(function(){
        ...
        var queryURL="${pageContext.request.contextPath}/likes/"+userId+"/xx";
        $likeTable = $('#like_table').bootstrapTable({
            url: queryURL,
            method: 'GET',
        ...
        $('#like_table').on("click","a[name='child']",function(){
            var parentId = $(this).data("id");
            $("#searchForm input[name='parentId']").val(parentId);
            $likeTable.bootstrapTable('refresh');
        });
        ...
        $("#toolbar").on("click","button",function(){
            ...
            if(ope=="delete"){
                //获取选中的
                var idArray = $likeTable.bootstrapTable('getAllSelections');
                idArray = idArray.map(function(item,index){
                    return item.id;
                });
                // delete  /likes/1,2,3,4
                $.ajax({
                    type:"post",
                    url:"${pageContext.request.contextPath}/likes/"+idArray.join(),
                    data:"_method=delete",
                ...
                window.location.href="${pageContext.request.contextPath}/likes/"+idArray[0].id+"/edit";
            }
        });
    });
</script>
